                ...
                <table class="w-full text-sm hidden md:table">
                    <thead class="bg-muted/50 border-b">
                        <tr>
                            ...
                            <th class="h-12 px-4 text-left font-bold text-muted-foreground w-[180px]">期限 / 担当</th>
                        ...
                        <tr v-for="project in projects?.data" :key="project.id" class="hover:bg-muted/10 transition-colors group">
                            ...
                            <td class="p-4">
                                <div class="flex flex-col gap-1.5 text-[11px]">
                                    ...
                                    <div class="flex items-center gap-2 text-muted-foreground">
                                        ...
                                        <span>{{ project.manager?.name || '担当未設定' }}</span>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="p-4 text-right">
                                <div class="flex flex-col">
                                    <span class="text-[10px] font-bold text-muted-foreground uppercase leading-none mb-1">Amount</span>
                    ...
                    <div v-for="project in projects?.data" :key="project.id" class="p-4 active:bg-muted/50 transition-colors">
                        <Link :href="`/project/edit/${project.id}`" class="block space-y-4">
                            <div class="flex justify-between items-start">
                                ...
                                <span class="px-2 py-0.5 rounded-full text-[10px] font-bold border bg-blue-50 text-blue-700">
                                    {{ project.status_label || '進行中' }}
                                </span>
                            </div>
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="flex flex-wrap gap-1.5">
                                    ...
                                    <span v-if="project.bidding_qualifications" class="px-2 py-0.5 rounded bg-blue-50 text-blue-700 text-[10px] font-bold border border-blue-100">{{ project.bidding_qualifications }}</span>
                                </div>
                                ...
                                <div class="grid grid-cols-2 gap-2 pt-2 border-t border-dashed">
                                    ...
                                    <div class="text-[10px] space-y-1 text-right">
                                        ...
                                        <div class="font-bold text-primary text-sm tabular-nums">{{ formatCurrency(project.amount) }}</div>
                                    </div>
                                </div>
                            </div>
                        </Link>
